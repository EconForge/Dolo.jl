struct CompletePolyDR{S<:Grid,T<:RandomGrid,nx} <: AbstractDecisionRule{S,T,nx}
    grid_exo::S
    grid_endo::T
...
function CompletePolyDR(
        grid_exo::EmptyGrid, grid_endo::RandomGrid{ns},
        ::Union{Val{nx},Type{Val{nx}}}, order::Int=3
    ) where ns where nx
    coeffs = [Array{Float64}(BM.n_complete(ns, order), nx)]
    CompletePolyDR{EmptyGrid,RandomGrid{ns},nx}(grid_exo, grid_endo, coeffs, order)
end
...
function set_values!(
        dr::CompletePolyDR{<:G,<:RandomGrid,nx},
...
function evaluate(dr::CompletePolyDR{<:EmptyGrid,RandomGrid{d}}, points::Vector{Point{d}}) where d
    N = length(points)
...
function CompletePolyDR(
        grid_exo::S, grid_endo::RandomGrid{ns},
        ::Union{Val{nx},Type{Val{nx}}}, order::Int=3
    ...
    CompletePolyDR{S,RandomGrid{ns},nx}(grid_exo, grid_endo, coeffs, order)
end
