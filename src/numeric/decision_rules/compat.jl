...
evaluate(dr::AbstractDecisionRule{EmptyGrid, <:CartesianGrid}, z::AbstractVector) = vec(evaluate(dr,z'))
evaluate(dr::AbstractDecisionRule{EmptyGrid, <:CartesianGrid}, i::Int, y::Union{AbstractVector,AbstractMatrix}) = evaluate(dr,y) # kind of nonsensical
evaluate(dr::AbstractDecisionRule{EmptyGrid, <:CartesianGrid}, x::Union{AbstractVector,AbstractMatrix}, y::Union{AbstractVector,AbstractMatrix}) = evaluate(dr,y)
...
function evaluate(dr::AbstractDecisionRule{CartesianGrid{d1}, CartesianGrid{d2}, n_x}, x::AbstractMatrix, y::AbstractMatrix) where d1 where d2 where n_x
    ...
    yy = to_LOP(Point{d1}, y)
    res = evaluate(dr,xx,yy)
    return reinterpret(Float64, res, (n_x, N))'

end
#
evaluate(dr::AbstractDecisionRule{<:CartesianGrid, <:CartesianGrid}, x::AbstractVector, y::AbstractMatrix) = evaluate(dr, repmat(x', size(y,1)), y)
evaluate(dr::AbstractDecisionRule{<:CartesianGrid, <:CartesianGrid}, x::AbstractVector, y::AbstractVector) = evaluate(dr, vector_to_matrix(x), vector_to_matrix(y))
...
evaluate(dr::AbstractDecisionRule{<:CartesianGrid, <:CartesianGrid}, z::AbstractVector) = vec(evaluate(dr,vector_to_matrix(z)))
evaluate(dr::AbstractDecisionRule{CartesianGrid{d1}, CartesianGrid{d2}}, i::Int, y::Union{AbstractVector,AbstractMatrix}) where d1 where d2  = evaluate(dr,node(dr.grid_exo,i), y)
...
function evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2},n_x}, i::Int, z::AbstractMatrix) where n_x where d1 where d2
    N = size(z,1)
    assert(size(z,2)==d2)
...
evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2}, n_x}, i::Int, x::AbstractVector) where n_x where d1 where d2 = vec(evaluate(dr,i,vector_to_matrix(x)))
function evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2}, n_x}, inds::Vector{Int}, x::AbstractMatrix) where n_x where d1 where d2
    l = [evaluate(dr,inds[n],x[n,:])' for n=1:size(x,1)]
    vcat(l...)
end

function evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2}, n_x}, inds::Vector{Int}, x::AbstractVector) where n_x where d1 where d2
