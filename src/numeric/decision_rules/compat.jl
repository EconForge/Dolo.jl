...
function evaluate(dr::AbstractDecisionRule{EmptyGrid, CartesianGrid{d}, n_x}, z::AbstractMatrix{Float64}) where n_x where d
    N = size(z,1)
    assert(size(z,2)==d)
    points = reinterpret(Point{d}, z', (N,))
...
function evaluate(dr::AbstractDecisionRule{CartesianGrid{d1}, CartesianGrid{d2}, n_x}, x::AbstractMatrix, y::AbstractMatrix) where d1 where d2 where n_x
    N = size(x,1)
    assert(size(y,1)==N)
    xx = reinterpret(Point{d1}, x', (N,))
    yy = reinterpret(Point{d2}, y', (N,))
...
function evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2},n_x}, i::Int64, z::AbstractMatrix) where n_x where d1 where d2
    N = size(z,1)
    assert(size(z,2)==d2)
    points = reinterpret(Point{d2}, z', (N,))
    out = evaluate(dr,i,points)
    reinterpret(Float64, out, (n_x, N))'
end

evaluate(dr::AbstractDecisionRule{UnstructuredGrid{d1}, CartesianGrid{d2}}, i::Int, x::AbstractVector) where d1 where d2 = vec(evaluate(dr,vector_to_matrix(x)))
