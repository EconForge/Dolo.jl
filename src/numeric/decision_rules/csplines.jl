@compat const CubicSplineDR{S<:Grid,T<:Grid,nx} = DecisionRule{S,T,nx,<:Vector{<:Array{Float64}}}
...
function set_values!(dr::CubicSplineDR{<:CartesianGrid, <:CartesianGrid}, values::Array{Array{Float64,2},1})
    ...
    nx = size(values[1], 2)
    vv = zeros(prod(dr.grid_exo.n), prod(dr.grid_endo.n), nx)
    for n=1:size(vv, 1)
        # ah, if only Julia had chosen C-order arrays !
        vv[n, :, :] = values[n]
    end
    vv = reshape(vv, prod(dr.grid_endo.n)*prod(dr.grid_exo.n), nx)
...
function DecisionRule{nx}(grid_exo::UnstructuredGrid, grid_endo::CartesianGrid, ::Union{Val{nx},Type{Val{nx}}})
    # hmm kind of silently assuming we have cartesian grid
    orders = grid_endo.n
    coeffs = [zeros(nx, (orders+2)...) for i in 1:n_nodes(grid_exo)]
    return DecisionRule(grid_exo, grid_endo, coeffs, Val{nx})
end
