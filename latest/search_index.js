...

{
    ...
    "title": "YAML format",
    "category": "section",
    "text": "YAML stands for Yet Another Markup Language. It is a serialization language that allows complex data structures in a human-readable way. Atomic elements are floats, integers and strings. An ordered list can be defined by separating elements with commas and enclosing them with squere brackets:[1,2,3]Equivalently, it can be done on several line, by prepending - to each line- 'element'\n- element         # quotes are optional there is no ambiguity\n- third element   # this is interpreted as ``'third element'``Associative arrays map keys to (simple strings to arbitrary values) as in the following example:{age: 18, name: peter}Mappings can also be defined on severaly lines, structures can be nested by using indentation (use spaces no tabs):age: 18\nname: peter\noccupations:\n  - school\n  - guitar\nfriends:\n  paula: {age: 18}The correspondance between the yaml definition and the resulting Julia object is very transparent. YAML mappings and lists are converted to Julia dictionaries and arrays respectiveley.Special objects from the Dolo language can be created by adding a tag to yaml nodes as in the following examples:- !AR1:\n    rho: 0.9\n    Sigma: [[0.1]]or- !Product:\n     !AR1:\n        rho: 0.9\n        Sigma: [[0.1]]\n     !AgingProcess:\n          mu: 0.01     # death probability\n          K: 10        # number of agesAny model file must be syntactically correct in the Yaml sense, before the content is analysed further. More information about the YAML syntax can be found on the YAML website, especially from the language specification."
},
...

{
    ...
    "category": "section",
    "text": "This section is introduced by the symbols keyword. All symbols appearing in the model must be defined there.Symbols must be valid Julia identifiers (alphanumeric not beginning with a number) and are case sensitive. Greek letters (save for lambda which is a keyword) are recognized. Subscripts and superscripts can be denoted by _ and __ respectively. For instance beta_i_1__d will be pretty printed as beta_i1^d.Symbols are sorted by type as in the following example:symbols:\n  variables: [a, b]\n  shocks: [e]\n  parameters: [rho]Note that each type of symbol is associated with a symbol list (as [a,b]).noteA common mistake consists in forgetting the commas, and use spaces only. This doesn't work since two symbols are recognized as one.The expected types depend on the model that is being written:For Dynare models, all endogenous variables must be listed as   variables with the exogenous shocks being listed as shocks (as in   the example above).noteThe variables, shocks and parameters keywords correspond to the var, varexo and param keywords in Dynare respectively.Global models require the definition of the parameters, and to providea list of states and controls. Mixed states model also require markov_states that follow a discrete markov chain, while continuous states model need to identify the i.i.d shocks that hit the model. If the corresponding equations are given (see next subsection) optional symbols can also be defined. Among them: values, expectations."
},
...

{
    ...
    "title": "Transitions",
    "category": "section",
    "text": "- name: `transition`\n- short name: `g`Transitions are given by a function g such that at all times:s_t = g(m_t-1 s_t-1 x_t-1 m_t)where m_t is a vector-valued exogenous process.noteIn the RBC model, the vector of endogenous states is s_t=(a_tk_t). The transitions are (note )a_t = rho a_t-1 + epsilon_tk_t = (1-delta)k_t-1 + i_t-1If epsilon_t follow a normal law with variance sigma_epsilon, the yaml file is amended with:symbols:\n    states: [a,k]\n    controls: [i]\n    exogenous: [epsilon]\n    ...\nequations:\n    transition:\n        a = rho*a(-1) + e\n        k = k(-1)*(1-delta) + i(-1)\nexogenous: !Normal\n    Sigma: [[sigma_epsilon]]Note that transition equations must list states in declaration order. Also, in this example productivity process a_t is essentially an exogenous AR1 process. Declaring it as such (instead of epsilon), provides additionalinformation to the solvers and can lead to faster solution time."
},
